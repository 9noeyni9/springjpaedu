...
public class EmpDAO {
	...
	public List<EmpFreqVO> findByEmpFreqVO() {
		TypedQuery<EmpFreqVO>  q = em.createQuery("SELECT new jpamvcexam.model.vo.EmpFreqVO(t.empno, t.ename, t.hiredate, t.sal, t.deptno) FROM EmpVO t", EmpFreqVO.class);/*새로운 vo객체 만들어*/
		List<EmpFreqVO> resultList = q.getResultList();/*원하는 컬럼만 추출할 수 있음*/
		return resultList;
	}
	...
	public Object[] getGroupFunc() {	
		Query query = em.createQuery("SELECT sum(t.sal), max(t.sal), min(t.sal) FROM EmpVO t");/*tyedquery아니고 그냥 쿼리로 처리한 다음에*/
		Object[] result = (Object[])query.getSingleResult();/*하나의 쿼리에 대해 각각의 데이터를 오브젝트 배열로 받아옴*/
		return result;
	}	
